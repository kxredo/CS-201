...
namespace array {
    ...
    bool Graph::isCompleteBipartite() const {
            ...
            for (int u = 0; u < vertexCount; ++u) {
                if (edges[v][u] > 0) { // There is an edge between v and u
                    if (set[u] == -1) {
                        ...
                        // If u is in the same set as v, then the graph is not bipartite
                        delete[] set;
                        return false;
                    }
                }
        ...
        // Check if the graph is complete bipartite
        for (int i = 0; i < vertexCount; ++i) {
            for (int j = 0; j < vertexCount; ++j) {
                if (i != j && set[i] != set[j] && edges[i][j] == 0) {
                    // If there is no edge between vertices of different sets, return false
                    delete[] set;
                    return false;
                }
            }
